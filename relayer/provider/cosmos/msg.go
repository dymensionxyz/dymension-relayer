...
func (cm CosmosMessage) Seq() uint64 {
	if m, ok := cm.Msg.(*chantypes.MsgRecvPacket); ok {
		return m.Packet.Sequence
	}
	if m, ok := cm.Msg.(*chantypes.MsgTimeout); ok {
		return m.Packet.Sequence
	}
	if m, ok := cm.Msg.(*chantypes.MsgAcknowledgement); ok {
		return m.Packet.Sequence
	}
