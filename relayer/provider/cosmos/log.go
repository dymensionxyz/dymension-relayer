...
func getFeePayer(tx *typestx.Tx) string {
	...
	case *channeltypes.MsgAcknowledgement:
		return firstMsg.Signer
	default:
		// can't use GetSigners because the method uses AccAddressFromBech32()
		// and AccAddressFromBech32() makes the following call:
		// bech32PrefixAccAddr := GetConfig().GetBech32AccountAddrPrefix()
		// and GetConfig uses sdkConfig which is for a scop by
		// done := cc.SetSDKContext() and then done()
		// therefore, the config might be obsolete and the call might fail
		//return firstMsg.GetSigners()[0].String()
		signerStr := ""
		val := reflect.ValueOf(tx.GetMsgs()[0]).Elem()
		for i := 0; i < val.NumField(); i++ {
			if val.Type().Field(i).Name == "Signer" {
				signer := reflect.Indirect(val).FieldByName("Signer")
				signerStr = string(signer.String())
			}
		}
		return signerStr
	}
