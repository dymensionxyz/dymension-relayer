...
func (r *RelayMsgs) IsMaxTx(msgLen, txSize uint64) bool {
	return IsMaxTx(r.MaxMsgLength, r.MaxTxSize, msgLen, txSize)
}
...
func (r *RelayMsgs) send(
	...
	defer wg.Done()
	Send(ctx, log, s, msgs, memo, successes, errors, r.MaxMsgLength, r.MaxTxSize)
}

func IsMaxTx(MaxMsgLength, MaxTxSize, msgLen, txSize uint64) bool {
	return (MaxMsgLength != 0 && msgLen > MaxMsgLength) ||
		(MaxTxSize != 0 && txSize > MaxTxSize)
}
...
func Send(
	...
	for i, msg := range msgs {
		...
		if !IsMaxTx(MaxMsgLength, MaxTxSize, uint64(i)+1-batchStartIdx, txSize+uint64(len(bz))) {
